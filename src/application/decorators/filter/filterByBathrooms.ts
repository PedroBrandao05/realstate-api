...
export default class filterByBathrooms implements IFilterDecorator {
    constructor (
    ...
    async leach(value: number, previous?: string[]): Promise<string[]> {
        const filteredInfrastructureDetails = await infrastructureDetailsRepository.findByBathroomsNumber(value)
        const propertiesFound = filteredInfrastructureDetails.map((infrastructureDetail) => infrastructureDetail.propertyId)
        if (!filteredInfrastructureDetails.length) throw new ApplicationError(`There are no properties with ${value} bathrooms`, 400)
        if (!previous) {
