...
export default class filterBySaleCost implements IFilterDecorator {
    ...
    async leach(values:  {max: number, min: number}, previous?: string[]): Promise<string[]> {
        const filteredFinancialDetails = await financialDetailsRepository.findBySaleCost(values.max, values.min)
        const propertiesFound = filteredFinancialDetails.map((financialDetail) => financialDetail.propertyId)
        if (!filteredFinancialDetails.length) throw new ApplicationError("There are no properties on available for this cost range", 400)
        if (!previous) {
